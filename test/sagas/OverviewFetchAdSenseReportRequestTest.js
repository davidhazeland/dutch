...
test('Overview Fetch AdSense Report Request saga', assert => {
  ...
  assert.deepEqual(actual[0], expected[0],
    'should wait for OVERVIEW_FETCH_ADSENSE_REPORTS_REQUEST action');
  ...
  assert.deepEqual(actual[1], expected[1],
    'should fork request when catch OVERVIEW_FETCH_ADSENSE_REPORTS_REQUEST action');
  ...
  assert.deepEqual(actual[3], expected[3],
    'then continue waiting for OVERVIEW_FETCH_ADSENSE_REPORTS_REQUEST action');
...
test('Overview Fetch AdSense Report Request saga: request generator', nest => {
  ...
  nest.test('...without error', assert => {
    ...
    assert.deepEqual(actual[1], expected[1],
      'then dispatch OVERVIEW_FETCH_ADSENSE_REPORTS_SUCCESS action');
  ...
  nest.test('...with error', assert => {
    ...
    assert.deepEqual(actual[1], expected[1],
      'then dispatch OVERVIEW_FETCH_ADSENSE_REPORTS_FAILURE action');
...
test('Overview Fetch AdSense Report Request saga: autoUpdate generator', nest => {
  ...
  nest.test('...request interval', assert => {
    ...
    assert.deepEqual(actual[0], expected[0],
      'should wait for OVERVIEW_STOP_FETCH_ADSENSE_REPORTS action or timeout');
    ...
    assert.deepEqual(actual[2], expected[2],
      'then continue waiting for OVERVIEW_STOP_FETCH_ADSENSE_REPORTS action or timeout');
  ...
  nest.test('..with stop action', assert => {
    ...
    assert.deepEqual(actual[1], expected[1],
      'should exit when catch OVERVIEW_STOP_FETCH_ADSENSE_REPORTS action');
